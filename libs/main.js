...
Proxy.prototype.listen = function (options, callback = e => { }) {
	...
	this.sslCaDir = options.sslCaDir || path.resolve(process.cwd(), '.http-proxy');
	ca.create(this.sslCaDir, function (err, ca) {
		...
		if (self.forceSNI) {
			self._createHttpsServer({}, function (port, httpsServer) {
		...
		self.onCertificateRequired(hostname, function (err, files) {
			...
			async.auto({
				...
				if (self.forceSNI && !hostname.match(/^[\d\.]+$/)) {
					...
					self._createHttpsServer(results.httpsOptions, function (port, httpsServer) {
